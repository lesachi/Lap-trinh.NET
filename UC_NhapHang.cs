...
namespace BookStore
{
    public partial class UC_NhapHang : UserControl
    {
        ...
        private Dictionary<string, string> maSachToTenSachMap = new Dictionary<string, string>(); // Map MaSach to TenSach
        private string currentSoHDN = null; // Theo dõi SoHDN đang nhập
        public UC_NhapHang()
        {
            InitializeComponent();
            InitializeDataTable();
        ...
        private void InitializeDataTable()
        {
            ...
            // Đặt chiều rộng cho các cột "Tên sách" và "Tên nhà cung cấp"
            ...
            using (SqlConnection conn = Database.GetConnection())
            {
                try
                {
                  // conn.Open();
                    ...
                    using (SqlCommand cmd = new SqlCommand(sqlNCC, conn))
                    {
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                    ...
                    // Load Sach data and create mapping
                    string sqlSach = "SELECT MaSach, TenSach FROM KhoSach";
                    using (SqlCommand cmd = new SqlCommand(sqlSach, conn))
                    {
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
        ...
        private void cmbMaSach_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                string selectedMaSach = cmbMaSach.SelectedItem.ToString();
                if (maSachToTenSachMap.ContainsKey(selectedMaSach))
                {
                    txtTenSach.Text = maSachToTenSachMap[selectedMaSach];
                }
                else
                {
                    txtTenSach.Text = "";
                ...
                try
                {
                   // conn.Open();
                    string sql = @"
                        ...
                        ORDER BY CAST(SUBSTRING(p.SoHDN, 4, LEN(p.SoHDN)) AS INT)"; // Sắp xếp theo phần số của SoHDN
        ...
        private string GetNewSoHDN()
        {
            using (SqlConnection conn = Database.GetConnection())
            {
                ...
                {
                    MessageBox.Show("Lỗi khi sinh mã hóa đơn: " + ex.Message);
                    return "HDN001";
                }
            }
        }
        ...
        private void DataGridViewPhieuNhapHang_SelectionChanged(object sender, EventArgs e)
        {
            if (DataGridViewPhieuNhapHang.SelectedRows.Count > 0)
            {
                ...
                // Truy vấn thông tin chung của HoaDonNhap (MaNV, TenNV, MaNCC, TenNCC, NgayNhap)
                using (SqlConnection conn = Database.GetConnection())
                {
                    try
                    {
